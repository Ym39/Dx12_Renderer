...
PixelOutput PS(Output input)
{
	...
	float3 viewDirection = input.pos.xyz - eye;
	...
	output.color = result * shadowWeight;
	output.color.w = alpha;
	output.normal.rgb = float3((input.normal.xyz + 1.0f) / 2.0f);
	output.normal.a = depthFromLight;

	float y = dot(float3(0.299f, 0.587f, 0.114f), output.color.xyz);
	output.highLum = y > bloomThreshold ? output.color : 0.0f;
